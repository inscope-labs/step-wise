...
- Pending for 1.6.0: behavioral acceptance and regression tests run against a model (plan Phase 6; the linter checks structure, not behavior); the token-accurate size measurement with the real tokenizer; the example transcript; removing the `-dev` suffix and dating the release.
